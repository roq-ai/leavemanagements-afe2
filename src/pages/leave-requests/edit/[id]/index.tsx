...
import { getLeaveRequestById, updateLeaveRequestById } from 'apiSdk/leave-requests';
...
function LeaveRequestEditPage() {
  ...
  const handleSubmit = async (values: LeaveRequestInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateLeaveRequestById(id, values);
      mutate(updated);
      resetForm();
